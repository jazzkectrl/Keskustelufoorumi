...
function syotaKommentti($post_id, $kommentti_sisalto) {
    ...
    try {
        ...
        $sql = "INSERT INTO viihde_kommentit (kommentti_id, post_id, kommentti_sisalto, kommentti_pvm) VALUES (NULL, :post_id, :kommentti_sisalto, NOW())";
        $stmt = $conn->prepare($sql);
        //lLiitetään parametrit sql lausekkeeseen
...
function tuoKommentti($post_id) {
    ...
    try {
        ...
        $sql = "SELECT * FROM viihde_kommentit WHERE post_id = :post_id";
        $stmt = $conn->prepare($sql);
