...
function syotaPostaukset($nimi, $sisalto) {
    ...
    try {
        ...
        $sql = "INSERT INTO viihde_postaukset (nimi, sisalto, pvm) VALUES (:nimi, :sisalto, NOW())";
...
function tuoPostaukset() {
    ...
    try {
        ...
        $sql = "SELECT * FROM viihde_postaukset ORDER BY pvm DESC";
        $stmt = $conn->prepare($sql);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['nimi']) && !empty($_POST['sisalto'])) {
        ...
        if (syotaPostaukset($nimi, $sisalto)) {
            ...
            header("Location: viihde_keskustelu.php");
            exit(); 
        } else {
